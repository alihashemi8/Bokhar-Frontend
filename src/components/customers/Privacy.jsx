import { Lock, Eye, EyeOff, Smartphone, Monitor } from "lucide-react";
...
export default function SecurityPrivacy() {
  ...
  const toggleOtp = () => setOtpEnabled(!otpEnabled);
  ...
  const getDeviceIcon = (type) => {
    switch (type) {
      case "mobile": return <Smartphone size={22} className="text-blue-500" />;
      case "laptop": return <Monitor size={22} className="text-purple-500" />;
      case "desktop": return <Monitor size={22} className="text-green-500" />;
      default: return <Lock size={22} className="text-gray-500" />;
    }
  };
  ...
  return (
    <div dir="rtl" className="min-h-screen p-4 md:p-8 bg-gray-50">
      <div className="md:max-w-3xl md:mx-auto space-y-6 md:mt-15 mb-20 md:mb-0">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-5 hover:shadow-xl transition">
          <div className="flex items-center gap-3 mb-5">
            <p className="font-medium text-gray-800 text-lg">دستگاه‌ها و نشست‌ها</p>
          </div>
          <div className="space-y-4">
            {devices.map((d) => (
              ...
              >
                ...
                <button className="text-red-500 text-sm hover:underline">خروج</button>
              </div>
            ))}
          </div>
        </div>
